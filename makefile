...
# Set to auth endpoint for private Docker registry. WARNING: must be set to empty for Docker Hub
...
buildtag:
	...
	@ $(foreach tag,$(BUILDTAG_ARGS), docker tag -f $(IMAGE_ID) $(DOCKER_REGISTRY)/$(ORG_NAME)/$(REPO_NAME):$(tag).$(BUILD_TAG);)
	${INFO} "Tagging complete"

login:
	${INFO} "Logging in to Docker registry $$DOCKER_REGISTRY as user $$DOCKER_USER ..."
